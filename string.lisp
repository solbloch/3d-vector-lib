...
(defun string-update ()
  ...
  (do* ((last (car nodes) (car current))
        ...
        (r1 (sub (state-pos last) (state-pos (car current))) (sub (state-pos last) (state-pos (car current))))
        (r2 (sub (state-pos (cadr current)) (state-pos (car current))) (sub (state-pos (cadr current)) (state-pos (car current))))
        (r1len (len r1) (len r1))
        (r2len (len r2) (len r2))
        (delr1 (* k (- r1len nlen)) (* k (- r1len nlen)))
        (delr2 (* k (- r2len nlen)) (* k (- r2len nlen)))
        (dp1 (sub (state-vel last) (state-vel (car current))) (sub (state-vel last) (state-vel (car current))))
        (dp2 (sub (state-vel (cadr current)) (state-vel (car current))) (sub (state-vel (cadr current)) (state-vel (car current))))
        (const (/ .01d0 *dt*)))
    ...
    (let ((pendulums nil))
      (sdl:with-events ()
        ...
        (:mouse-motion-event (:state state :x x :y y :x-rel x-rel :y-rel y-rel)
                             ...
                             (setf (v3y (state-pos (car nodes))) (coerce y 'double-float)))
        (:mouse-button-down-event (:button button :state state :x x :y y)
                                  (cond ((eq button 4)
                                         (incf (v3y (state-pos (car nodes))) -8d0)))
                                  (cond ((eq button 5)
                                         (incf (v3y (state-pos (car nodes))) 8d0))))
